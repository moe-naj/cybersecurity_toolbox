...
author: Moe
last_edited_time: '2026-08-13 17:52:00.000'
---
 | **Tool** | **Scope** | **Details** | 
 | ---- | ---- | ---- | 
 | **Active Directory** |  |  | 
 ...
 | extundelete | Recovery | A command-line utility that recovers deleted files from ext3/ext4 filesystems by parsing the filesystem's journal and inode metadata to reconstruct files whose data blocks haven't yet been overwritten. | 
 ...
 | john | Secrets | An offline password-cracking tool that takes hashed credentials and attempts to recover the original plaintext by testing candidate passwords via wordlists, rules, or brute-force. | 
 ...
 | Bandit | Vulnerabilities | An open-source static analysis (SAST) tool that scans Python code for common security issues (hardcoded secrets, insecure functions, weak crypto, risky subprocess usage) before it ships. | 
 ...
 | dnSpy |  | dnSpy is a debugger and .NET assembly editor. It is used for decompiling and debugging .NET applications, which is particularly useful in reverse engineering .NET binaries to understand their functionality. | 
 | EvtxECmd |  | **EvtxECmd** is a fast, command-line forensic tool that parses Windows Event Log (`.evtx`) files into structured, human-readable output (CSV, JSON, XML, or SQLite) for analysis. Basic usage:`EvtxECmd.exe -f "C:\Logs\Security.evtx" --csv "C:\Out" --csvf Security.csv` | 
