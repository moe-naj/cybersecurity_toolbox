...
`\.\ADRecon.ps1 -Domain 'corp.local' -User 'administrator' -Password 'P@ssw0rd'` | 
 ...
 | fls | Disk | A Sleuth Kit command that lists files and directories in a disk image (including deleted entries), typically by walking a filesystem or a specific directory inode. | 
 | FTK Imager | Disk | FTK Imager is a digital forensics tool used to quickly create forensic disk/memory images and preview/extract files and artifacts without altering the original evidence. AD1 is the AccessData FTK logical evidence container format. | 
 | icat | Disk | A Sleuth Kit command that extracts a file’s contents from a disk image (by inode/MFT entry), commonly used to recover and carve out deleted files for analysis. | 
 | istat | Disk | A Sleuth Kit command that displays a filesystem's low-level metadata (timestamps, attributes, MFT/inode details) for a specific file, read directly from a disk image. | 
 | KAPE | Disk | KAPE (Kroll Artifact Parser and Extractor) is a DFIR tool that quickly collects key forensic artifacts (and can optionally run parsers on them) from a Windows system or disk image for triage. | 
 | libewf | Disk | Open-source library and toolset for reading and writing EnCase Expert Witness Format (`E01`) forensic disk images on Linux. | 
 | MFTECmd | Disk | MFTECmd is an Eric Zimmerman DFIR command-line tool that parses the NTFS Master File Table (`$MFT`) to extract file and directory metadata/timestamps for forensic timeline analysis. Example command: `MFTECmd.exe -f "C:\Users\Administrator\Desktop\$MFT" --csv "C:\Users\Administrator\Desktop"` | 
 | MFT Explorer | Disk | MFT Explorer is a GUI forensics tool for parsing and browsing the NTFS Master File Table ($MFT) to inspect file metadata/timelines and identify resident $DATA (resident files) stored directly in MFT records. | 
 | NTFS Log Tracker | Disk | A forensic utility that parses the NTFS transaction log (`$LogFile`) and optionally USN Journal (`$UsnJrnl`) to reconstruct file system operations like create, delete, rename, and move. | 
 | UAC | Disk | An incident response and forensic tool for Unix-like systems (Linux, macOS, Solaris, AIX, etc.) that collects artifacts, logs, memory info, running processes, network connections, and more into a structured output directory. | 
 | Velociraptor | Disk | An open-source DFIR endpoint monitoring, live response, and forensic investigation platform by Rapid7 that uses a query language called VQL to collect, hunt, and analyze forensic artifacts across endpoints at scale. | 
 | pdfid | Document | A quick PDF triage tool (Didier Stevens) that counts suspicious PDF keywords (e.g. `/JavaScript`, `/OpenAction`, `/AcroForm`) to flag a file as worth deeper analysis. | 
 | pdfparser | Document | Didier Stevens tool that parses and dumps individual PDF objects/streams (including decompressing and decoding filters) for manual inspection. | 
 | PDFStreamDumper | Document | Windows GUI tool for exploring PDF streams and objects with built-in JavaScript deobfuscation and shellcode analysis features. | 
 | peepdf | Document | Python-based interactive PDF analysis framework that parses object structure, decodes streams, and can extract/analyze embedded JavaScript in one tool. | 
 | 4n6 Outlook Forensics Analyzer | Email | 4n6 (Forensiksoft Outlook Forensics) is an email forensics tool used to parse and analyze Outlook mailbox data (PST/OST/MSG) to investigate messages, headers, attachments, and user activity for incidents like phishing/BEC. | 
 | Sublime Security | Email | Email security platform focused on detecting and responding to malicious or suspicious email using a detection engine and API-driven workflows (phishing, BEC, malware delivery). | 
 | awk | Filtering | Stream-processing tools to extract fields from text: `awk -F: '{print $1}' /etc/passwd` | 
 | grep | Filtering | Search and filter lines matching a pattern, using a regular expression (regex): `grep -i "error" /var/log/syslog` | 
 | sed | Filtering | Edit and transform text: `sed 's/foo/bar/g' file.txt` | 
 | sort | Filtering | Reorder lines in lexicographic, key-based, or numeric order for organization or preparation for deduplication: `sort -u hosts.txt` | 
...
[https://github.com/digitalsleuth/autoit-extractor](https://github.com/digitalsleuth/autoit-extractor) | 
 ...
 | Triage | Malware | [Tria.ge](http://tria.ge/) (Triage) is a public malware sandbox analysis platform that detonates submitted files/hashes in an instrumented environment and reports behavioral indicators, tags, and family classification. URL: [https://tria.ge/s](https://tria.ge/s) | 
 | unpac.me | Malware | Allows security analysts to safely detonate and analyze packed or obfuscated malware samples in a sandbox environment, extracting the actual malicious code to understand its behavior and indicators without manual unpacking. | 
 | x96dbg | Malware | Broken into both `x32dbg `and `x64dbg` , an open-source, feature-rich Windows debugger (Olly-style UI) for live dynamic analysis of 32-/64-bit binaries. | 
 | AVML | Memory | A Linux memory acquisition tool that captures a live RAM image for forensic analysis. | 
